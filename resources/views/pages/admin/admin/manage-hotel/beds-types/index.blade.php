...
<div class="h-auto px-4 pt-6">
    <div class="flex justify-between">
        <div class="">
            <h1 class="text-[20px] font-bold">Bed List</h1>
        </div>
        <div class="flex">
            <form>
                <div class="flex mr-5">
            ...
            <label for="create" class="cursor-pointer modal">
                <label class="relative modal-box" for="">
                    ...
                    <h3 class="text-[20px] font-bold border-b-2 pb-2 mb-5">Add New Bed Type</h3>

                    <form action="/hotel/admin/bed/store" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="">
                            @error('name')
                            {{ $message }}
                            ...
                            <button type="submit"
                                class="bg-[#4634FF] text-white w-full h-11 text-center align-center rounded-md">Submit</button>
                        </div>
                    </form>
                </label>
            </label>
            ...
            <tbody class="text-sm font-light text-gray-600 bg-white">
                ...
                <tr class="border border-gray-200 hover:bg-gray-100">
                    ...
                    <td class="flex justify-around px-6 py-4 text-center whitespace-normal ">
                        ...
                        <form method="POST" action="/hotel/admin/bed/{{ $bed->id }}/delete">
                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}
                ...
                <input type="checkbox" id="edit-{{ $bed->id }}" class="modal-toggle" />
                <div class="modal">
                    <div class="relative modal-box">
                        <label for="edit-{{ $bed->id }}" class="absolute btn btn-sm btn-circle right-2 top-2">✕</label>
                        <h3 class="text-[20px] font-bold border-b-2 pb-2 mb-5">Edit Form</h3>
                        <form action="/hotel/admin/bed/{{ $bed->id }}/edit" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="">
                                @error('name')
                                {{ $message }}
